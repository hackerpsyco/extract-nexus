...
from .models import User, Role, School, ClassSection, FacilitatorSchool, PlannedSession, DateType, Cluster, Enrollment, GroupedSession
...
def initialize_grouped_session_plans(classes, grouped_session_id):
    ...
    - Creates 150 sessions for EACH class in the group
    - All sessions are linked with the same grouped_session_id
    - When querying, the system redirects to the primary class
    - Creates a GroupedSession record to permanently track the grouping
    """
    if not classes:
    ...
    try:
        with transaction.atomic():
            ...
            # Create 150 sessions for EACH class in the group
            # This ensures each class can query its own sessions
            # The grouped_session_id links them together
...
def supervisor_facilitator_detail(request, facilitator_id):
    ...
    context = {
        ...
        'end_date': end_date,
        'lesson_date_filter': lesson_date_filter,
...
@login_required
@supervisor_required
def set_active_day(request):
    """Set active day for a class - marks all previous days as conducted"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Invalid request method'})
    ...
    from django.utils import timezone
    from datetime import timedelta
    from .models import ActualSession, SessionStatus, CalendarDate, DateType
    ...
    try:
        ...
        if not class_id or not active_day:
            return JsonResponse({'success': False, 'error': 'Missing class_id or active_day'})
        
        try:
            active_day = int(active_day)
        except (ValueError, TypeError):
            return JsonResponse({'success': False, 'error': 'Invalid day number'})
        
        if active_day < 1 or active_day > 150:
            return JsonResponse({'success': False, 'error': 'Day must be between 1 and 150'})
        
        class_section = get_object_or_404(ClassSection, id=class_id)
        
        # Get all planned sessions for this class that need to be marked as conducted
        planned_sessions = PlannedSession.objects.filter(
            class_section=class_section,
            day_number__lt=active_day
        ).order_by('day_number')
        
        # Get the active day's planned session
        active_planned_session = PlannedSession.objects.filter(
            class_section=class_section,
            day_number=active_day
        ).first()
        
        if not active_planned_session:
            return JsonResponse({'success': False, 'error': f'Day {active_day} not found for this class'})
        
        today = timezone.now().date()
        created_count = 0
        
        with transaction.atomic():
            # For each session before active_day, create ActualSession with appropriate past date
            # This simulates that these sessions were conducted on previous days
            sessions_to_create = []
            
            for session in planned_sessions:
                # Check if ActualSession already exists for this session
                existing = ActualSession.objects.filter(
                    planned_session=session
                ).exists()
                
                if not existing:
                    # Calculate a past date for this session
                    # Days are numbered 1-150, so day 1 was (active_day - 1) days ago
                    days_ago = active_day - session.day_number
                    ...
                    sessions_to_create.append(
                        ActualSession(
                            ...
                            remarks='Auto-marked as conducted when setting active day'
                        )
                    )
            
            # Bulk create ActualSession records
            if sessions_to_create:
                ActualSession.objects.bulk_create(sessions_to_create, batch_size=50)
                created_count = len(sessions_to_create)
            
            # Update existing sessions that aren't marked as conducted
            ActualSession.objects.filter(
                planned_session__in=planned_sessions
            ).exclude(status=SessionStatus.CONDUCTED).update(
                status=SessionStatus.CONDUCTED,
                remarks='Auto-marked as conducted when setting active day'
            )
            
            # Delete any ActualSession for today that's not the active day
            # This ensures get_next_pending_session will find the active day
            ActualSession.objects.filter(
                planned_session__class_section=class_section,
                date=today
            ).exclude(
                planned_session=active_planned_session
            ).delete()
            
            # Create or update ActualSession for the active day for today
            # Use HOLIDAY status so it's recognized as "today's session" but not conducted
            today_actual_session, created = ActualSession.objects.get_or_create(
                planned_session=active_planned_session,
                date=today,
                defaults={
                    'status': SessionStatus.HOLIDAY,
                    'remarks': 'Active day set by supervisor'
                }
            )
            
            # If it already existed, update it to HOLIDAY status
            if not created:
                today_actual_session.status = SessionStatus.HOLIDAY
                today_actual_session.remarks = 'Active day set by supervisor'
                today_actual_session.save()
            
            # Update CalendarDate to point to the active day
            # Get or create supervisor's calendar
            from .models import SupervisorCalendar
            supervisor_calendar, _ = SupervisorCalendar.objects.get_or_create(
                supervisor=request.user
            )
            
            # Find all CalendarDate records for today that include this class
            ...
            # Delete old calendar dates for this class on today
            calendar_dates_today.delete()
            
            # Create new CalendarDate for today pointing to the active day
            calendar_date = CalendarDate.objects.create(
                calendar=supervisor_calendar,
                date=today,
                date_type=DateType.SESSION,
            ...
            calendar_date.class_sections.add(class_section)
            
            # If this is a grouped session, add all grouped classes
            if active_planned_session.grouped_session_id:
                grouped_classes = PlannedSession.objects.filter(
                    grouped_session_id=active_planned_session.grouped_session_id,
                    day_number=active_day
                ).values_list('class_section_id', flat=True).distinct()
                
                for grouped_class_id in grouped_classes:
                    calendar_date.class_sections.add(grouped_class_id)
                    
                    # Also create/update ActualSession for today for each grouped class
                    grouped_class = ClassSection.objects.get(id=grouped_class_id)
                    grouped_planned_session = PlannedSession.objects.filter(
                        class_section=grouped_class,
                        day_number=active_day
                    ).first()
                    
                    if grouped_planned_session:
                        # Delete any other ActualSession for today for this grouped class
                        ActualSession.objects.filter(
                            planned_session__class_section=grouped_class,
                            date=today
                        ).exclude(
                            planned_session=grouped_planned_session
                        ).delete()
                        
                        # Create or update ActualSession for the active day for today
                        grouped_actual_session, created = ActualSession.objects.get_or_create(
                            planned_session=grouped_planned_session,
                            date=today,
                            defaults={
                                'status': SessionStatus.HOLIDAY,
                        ...
                        if not created:
                            grouped_actual_session.status = SessionStatus.HOLIDAY
                            grouped_actual_session.remarks = 'Active day set by supervisor'
                            grouped_actual_session.save()
        
        return JsonResponse({
            'success': True,
            'message': f'Active day set to {active_day}. Marked {created_count} previous days as conducted.',
        ...
        logger.error(f'Error setting active day: {str(e)}')
