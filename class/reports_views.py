...
def get_students_report_data(filters, date_filter):
    """Get students report data - optimized for performance with aggregation"""
    try:
        from django.db.models import Count, Q, Max
        
        # Build the query step by step
        queryset = Enrollment.objects.filter(is_active=True).select_related(
            'student', 'class_section', 'school'
        )
        ...
        # Get enrollments with pagination (500 per batch)
        enrollments = queryset[:500]
        
        # Get all session counts per class in ONE query
        ...
        session_counts_dict = {
            item['planned_session__class_section_id']: item['total_sessions']
        ...
        for enrollment in enrollments:
            try:
                # Get attendance data from dict (no DB query)
                att_data = attendance_dict.get(enrollment.id, {})
                present_count = att_data.get('present_count', 0)
                absent_count = att_data.get('absent_count', 0)
                last_marked_at = att_data.get('last_marked_at')
                
                # Get total sessions from dict (no DB query)
                ...
                last_session = last_marked_at.strftime('%Y-%m-%d') if last_marked_at else 'N/A'
...
def get_facilitators_report_data(filters, date_filter):
    """Get facilitators report data - optimized with aggregation"""
    try:
        ...
        # Get all facilitator schools in ONE query
        facilitator_schools = FacilitatorSchool.objects.filter(
            facilitator__in=facilitators
        ).values('facilitator_id').annotate(
            schools_count=Count('school_id', distinct=True)
        )
        
        # Convert to dict for fast lookup
        schools_dict = {
            item['facilitator_id']: item['schools_count']
            for item in facilitator_schools
        }
        
        # Get all sessions conducted in ONE query
        sessions_conducted_data = ActualSession.objects.filter(
            facilitator__in=facilitators,
            status=SessionStatus.CONDUCTED
        ).values('facilitator_id').annotate(
        ...
        # Convert to dict for fast lookup
        sessions_dict = {
            item['facilitator_id']: item
            for item in sessions_conducted_data
        }
        
        # Get all feedback ratings in ONE query
        feedback_ratings = SessionFeedback.objects.filter(
            actual_session__facilitator__in=facilitators
        ...
        # Convert to dict for fast lookup
        ratings_dict = {
            item['actual_session__facilitator_id']: item['avg_rating']
            for item in feedback_ratings
        }
        ...
        for facilitator in facilitators:
            try:
                ...
                schools_count = schools_dict.get(facilitator.id, 0)
                sessions_info = sessions_dict.get(facilitator.id, {})
                sessions_conducted = sessions_info.get('sessions_count', 0)
                last_session_date = sessions_info.get('last_session_date')
                avg_rating = ratings_dict.get(facilitator.id)
                
                last_active = last_session_date.strftime('%Y-%m-%d') if last_session_date else 'N/A'
...
def get_attendance_report_data(filters, date_filter):
    """Get attendance report data - optimized with aggregation"""
    try:
        from django.db.models import Count, Q, Max
        ...
        sessions = queryset.select_related(
            ...
            'facilitator'
        ).order_by('-date')[:500]  # Limit to 500 for performance
        
        # Get all enrollment counts per class in ONE query
        class_enrollment_counts = Enrollment.objects.filter(
            is_active=True
        ...
        enrollment_dict = {
            ...
            for item in class_enrollment_counts
        }
...
def get_sessions_report_data(filters, date_filter):
    """Get sessions report data - optimized with select_related"""
    try:
        # Build the query
...
def get_feedback_report_data(filters, date_filter):
    """Get feedback report data - optimized with select_related"""
    try:
