...
class SessionSequenceCalculator:
    ...
    def get_next_pending_session(class_section: ClassSection, facilitator: User = None) -> Optional[PlannedSession]:
        ...
        For grouped sessions, returns the shared session from the primary class
        ...
        try:
            ...
            # If so, get the primary class (first class in the group)
            first_planned_session = PlannedSession.objects.filter(
                class_section=class_section,
                is_active=True
            ).order_by('day_number').first()
            
            if first_planned_session and first_planned_session.grouped_session_id:
                # This class is part of a grouped session
                # Get the primary class (the one with all 150 sessions)
                primary_session = PlannedSession.objects.filter(
                    grouped_session_id=first_planned_session.grouped_session_id,
                    day_number=1
                ).select_related('class_section').order_by('id').first()
                
                if primary_session:
                    # Use the primary class for session lookup
                    class_section = primary_session.class_section
                    logger.info(f"Grouped session detected: using primary class {class_section} for {first_planned_session.class_section}")
    ...
    def calculate_progress(class_section: ClassSection) -> ProgressMetrics:
        ...
        try:
            ...
            if first_planned and first_planned.grouped_session_id:
                ...
                if primary_session:
                    class_section = primary_session.class_section
                    logger.info(f"Grouped session detected in calculate_progress: using primary class {class_section}")
    ...
    def conduct_session(planned_session: PlannedSession, facilitator: User, 
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"Session started (PENDING): {planned_session} by {facilitator}")
